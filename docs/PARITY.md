...
| macOS (`macos/`) | Local-build Mac operator on the same spine and the same portable core: BLE pair → CoreWLAN join of `OSMO-*` → UDP 9004 + TCP 7001 → HEVC on `AVSampleBufferDisplayLayer`. Deployment target macOS 26, Swift 5 language mode like the iOS shell, non-sandboxed on purpose. | **Exception — personal/local builds only, not App Store.** `NEHotspotConfiguration` does not exist on Mac: `WiFiJoiner` uses CoreWLAN `associate` (with a manual join + path-probe fallback); SSID read needs Location Services. Phone-battery cluster and device-orientation rails are iPhone-only and neutral on Mac; media library is a placeholder; Frame.io OAuth untested. No TestFlight/Play train. | **physical** on a Mac with the camera |
