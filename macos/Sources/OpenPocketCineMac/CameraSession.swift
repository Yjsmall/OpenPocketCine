...
final class CameraSession {
    ...
    var currentShootingMode: ShootingMode? {
        ...
        let raw = UInt8(status.shootingMode)
        // Nano Photo is raw `0x05`, which has no ShootingMode case (`photo`
        // carries the Pocket 4 encoding `0x17`). Map it so the smart shutter
        // fires a still instead of record (which answers `DF` in Photo mode).
        if raw == 0x05, connectedCamera?.model.family == .nano { return .photo }
        return ShootingMode(rawValue: raw)
    }

    /// Switch to the body's Photo mode: `.photo` (`0x17`) on a Pocket, raw
    /// `0x05` on a Nano — the case encoding answers `EE` there.
    func setPhotoMode() {
        guard connectedCamera?.model.family == .nano else {
            setShootingMode(.photo)
            return
        }
        let previous = status.shootingMode
        status.shootingMode = 0x05
        fireCamera(
            Commands.setShootingMode(raw: 0x05) ?? Commands.setShootingMode(.photo),
            name: "Photo (Nano)",
            onFail: { [weak self] in self?.status.shootingMode = previous })
    }
